...
class EvalCallback():
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        #---------------------------------------------------------#
        #   Converted image into an RGB image to prevent the grayscale image from reporting an error in prediction
        #---------------------------------------------------------#
        image       = cvtColor(image)
        
        #---------------------------------------------------------#
        #   resize the input image into (600, 600)
        #---------------------------------------------------------#
        ...
        with torch.no_grad():
            ...
            #-------------------------------------------------------------#
            #   Use the prediction result of the classifier to decode the ROI
            #-------------------------------------------------------------#
            results = self.bbox_util.forward(roi_cls_locs, roi_scores, rois, image_shape, input_shape, 
                                                    nms_iou = self.nms_iou, confidence = self.confidence)
            #--------------------------------------#
            #   If nothing detected return original image
            #--------------------------------------#
            if len(results[0]) <= 0:
                return 
